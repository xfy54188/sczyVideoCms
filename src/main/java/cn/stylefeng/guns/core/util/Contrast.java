...
public class Contrast
{
		...
		try
		{
			...
			for ( Field field : fields )
			{
				...
				if ( !o1.toString().equals( o2.toString() ) )
				{
					...
					{
						Object o1Warpper = DictFieldWarpperFactory.createFieldWarpper( o1,
								fieldWarpperMethodName );
						Object o2Warpper = DictFieldWarpperFactory.createFieldWarpper( o2,
								fieldWarpperMethodName );
	...
	public static String parseMutiKey( AbstractDictMap dictMap, String key, Map<String, String> requests )
	{
		...
		if ( key.contains( "," ) )
		{
			...
			for ( String item : keys )
			{
				...
				if ( fieldWarpperMethodName != null )
				{
					Object valueWarpper = DictFieldWarpperFactory.createFieldWarpper( value,
							fieldWarpperMethodName );
	 ...
	 * @Date 2019/3/6 10:19
	 */
	public static String getData( Row row, Integer num )
	{
		String result = null;
		// 将区域编号的cell中的内容当做字符串处理
		row.getCell( num ).setCellType( HSSFCell.CELL_TYPE_STRING );
		result = row.getCell( num ).getStringCellValue();
		boolean status = result.contains( "." );
		boolean isURL = result.contains( "http" );
		if ( status && !result.equals( "0" ) && !isURL)
		{
